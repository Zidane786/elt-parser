...
from etl_parser.describe.prompt import DESCRIPTION_STYLE_RULES
...
class AnalysisConfig(RunnerConfig):
    ...
    Attributes:
        ...
        exclude: Glob patterns excluding source files from AI work, applied after
            ``include``.
        min_ai_confidence: Minimum model-reported ``confidence`` (0-1) a proposal must
            carry to be accepted. Proposals below it are recorded as ``deferred`` with
            reason ``below_confidence_threshold`` and stay visible in the AI-only
            lineage and the change ledger. ``0.0`` (the default) accepts everything the
            evidence rules already allow.
        override_existing: Whether descriptions already generated by this package
            (``description_source`` ``ai`` or ``code``) may be regenerated. Text written
            or approved by a person (``human``, ``verified``, or any other source) is
            never overwritten, even when this is true.
    """
    ...
    min_ai_confidence: float = Field(default=0.0, ge=0, le=1)
...
class Evidence(StrictModel):
    ...
    Attributes:
        ...
        quote: Exact text quoted from the cited line range. Must be long enough to
            identify the construct it cites, so a one-character "quote" that trivially
            matches any source file cannot ground a proposal.
    """
    ...
    quote: str = Field(min_length=12, max_length=10000)
...
class ColumnProposal(StrictModel):
    ...
    Attributes:
        ...
        confidence: The model's own 0-1 self-assessment of this proposal. Never turns an
            edge into a deterministic one; it only lets a caller threshold AI output via
            :attr:`AnalysisConfig.min_ai_confidence`.
...
class TableProposal(StrictModel):
    ...
    Attributes:
        ...
        confidence: The model's own 0-1 self-assessment of this proposal.
        rationale: Short model-supplied reason behind ``confidence``.
        evidence: Source citation supporting this proposal.
    """
...
class TableDescriptionProposal(StrictModel):
    """One AI-proposed description for a catalog table.
...
class AnalysisResponse(StrictModel):
    ...
    Attributes:
        ...
        table_descriptions: Proposed table-level descriptions.
    ...
    table_descriptions: list[TableDescriptionProposal] = Field(
...
_UNKNOWN_COLUMN_ENTRY = {}
"""Stand-in for a column the baseline catalog does not contain yet.

An edge can target a column the catalog has not published (a newly proposed output), and
such a column still needs a description; an empty entry says exactly that.
"""

REGENERATABLE_SOURCES = frozenset({"ai", "code"})
"""Description sources this package may regenerate when overriding is enabled.

Any other value — including ``human``, ``verified`` and ``inherited`` from a human-written
upstream description — is left alone, so a person's text is never replaced by a model's.
"""


def _needs_description(entry, override_existing=False):
    """Decide whether a catalog table/column entry still needs a generated description.

    Args:
        entry: A catalog ``tables[]`` or ``schema[]`` dict, or ``None``.
        override_existing: Whether descriptions this package generated earlier
            (``description_source`` in :data:`REGENERATABLE_SOURCES`) may be replaced.

    Returns:
        bool: True when ``entry`` exists and has no description, or has one this package
        generated and ``override_existing`` is set. Human-authored text always returns
        False.
    """
    if entry is None:
        return False
    if not entry.get("description"):
        return True
    return override_existing and entry.get("description_source") in REGENERATABLE_SOURCES


def _write_description(entry, text, proposal, model):
    """Write a generated description and its model attribution onto a catalog entry.

    Args:
        entry: The catalog table/column dict to update in place.
        text: The description text to store.
        proposal: The proposal carrying ``confidence`` and ``rationale``.
...
def _catalog_tables(catalog):
    """Index an agent catalog's tables by ``dataset_id``.
    ...
    Returns:
        dict: Mapping from ``dataset_id`` to that table's dict, for tables that have one.
    """
    return {
        t["dataset_id"]: t
        for d in catalog.get("databases", [])
...
COLUMN_FACT_FIELDS = ("datatype", "is_partition", "is_categorical", "categorical_values")
"""Catalog column facts worth sending with a description request.

Datatype, partition role and known categorical values change how a column should be
described, and they come from the catalog rather than from the model.
"""


def _column_facts(column):
    """Extract the catalog facts a description prompt needs about one column.

    Args:
        column: A catalog ``schema[]`` entry, or ``None`` for a column the catalog does
            not contain yet.

    Returns:
        dict: The subset of :data:`COLUMN_FACT_FIELDS` present on ``column``; empty when
        ``column`` is ``None`` or carries none of them.
    """
    return {k: column[k] for k in COLUMN_FACT_FIELDS if column and k in column}


def _domain_for(doc, jobs):
    """Find the business domain declared for the product owning these jobs.

    Args:
        doc: The lineage document holding parsed products.
        jobs: The jobs parsed from the source file being analyzed.

    Returns:
        str | None: The declared domain when every product code among ``jobs`` agrees on
        one, else ``None``; the model is given no domain rather than an ambiguous one.
    """
    codes = {job.product for job in jobs} - {None}
    domains = {p.domain for p in doc.products if p.code in codes and p.domain}
...
def _identifier_tokens(text):
    """Split text into identifier-like tokens for whole-token grounding checks.

    Args:
        text: Arbitrary text, e.g. a source file, an expression or an evidence quote.

    Returns:
        set[str]: Every maximal run of identifier characters (letters, digits, ``_``),
        lowercased, so matching ignores case but never matches across a word boundary.
    """
    return {token.lower() for token in re.findall(r"[A-Za-z_][A-Za-z0-9_]*", text)}


def _validate_quote(evidence, column_name, expression):
    """Require an evidence quote to mention what the proposal claims it shows.

    A quote that cites neither the target column nor any identifier appearing in the
    proposed expression is not evidence for that proposal, however verbatim it is.

    Args:
        evidence: The :class:`Evidence` citation to check.
        column_name: Name of the target column the proposal produces.
        expression: The proposed transformation expression.

    Raises:
        ValueError: If the quote contains neither the target column name nor any
            identifier token from ``expression``.
    """
    tokens = _identifier_tokens(evidence.quote)
    if column_name.lower() in tokens:
        return
    if tokens & _identifier_tokens(expression):
        return
    raise ValueError("evidence_quote_unrelated_to_proposal")
...
def _validate_dataset(ident, known, source):
    ...
    # Whole-token match only: "db.trans" must not be grounded by "db.transactions".
    pattern = r"(?<![A-Za-z0-9_.])" + re.escape(literal) + r"(?![A-Za-z0-9_.])"
    if not re.search(pattern, source.text.replace('"', "").replace("`", "")):
        ...
        for item in items:
            try:
                ...
                if kind == "column":
                    ...
                    _validate_quote(item.evidence, item.target.name, item.expression)
                    if schema:
...
async def analyze_async(
    ...
    **scan_options,
    ...
    for source in index.files:
        ...
        description_targets = sorted(
            {
                ...
                and _needs_description(
                    catalog_columns.get(
                        (e.target.dataset_id, e.target.name), _UNKNOWN_COLUMN_ENTRY
                    ),
        ...
        # Table descriptions ride along with the columns of the same table. They never
        # justify a call on their own, so a file whose columns are all described stays
        # skipped exactly as before.
        table_targets = sorted(
            {
                dataset_id
                for dataset_id, _ in description_targets
                if _needs_description(catalog_tables.get(dataset_id), config.override_existing)
            }
        )
        ...
        prompt = {
            ...
            "description_targets": description_targets,
            "target_column_facts": {
                f"{ds}#{name}": _column_facts(catalog_columns.get((ds, name)))
                for ds, name in description_targets
            },
            "table_description_targets": table_targets,
            "table_descriptions": {
                dataset_id: catalog_tables.get(dataset_id, {}).get("description", "")
                for dataset_id in sorted({ds for ds, _ in description_targets} | set(table_targets))
            },
            ...
            "upstream_datatypes": {
                f"{ds}#{name}": c.get("datatype")
                for (ds, name), c in catalog_columns.items()
                if (ds, name) in {(r.dataset_id, r.name) for e in edges for r in e.sources}
            },
            "domain": _domain_for(doc, jobs),
        ...
        try:
            ...
            with observer.span("ai.complete", request_id=request_id):
                completion = await asyncio.wait_for(
                    runner.complete(
                        ...
                        "Every quote must be at least 12 characters and must contain the target "
                        "column name or an identifier from the expression. "
                        ...
                        "Describe every dataset id in table_description_targets in "
                        "table_descriptions, saying what one row represents and what the table "
                        "is used for. " + DESCRIPTION_STYLE_RULES + " "
                        "Every column, table, description and table_description entry must carry "
                        "confidence, a number from 0 to 1 stating how well the supplied evidence "
                        "supports that single item, and rationale, at most 500 characters saying "
                        "why. Report low confidence honestly instead of inflating it. "
            ...
            for edge in columns:
                ...
                low = (edge.provenance.ai_confidence or 0.0) < config.min_ai_confidence
                status = (
                    "unchanged"
                    if same
                    else "deferred"
                ...
                reason = (
                    ...
                    if low and not same
                    else "preserve_exact_baseline"
                    if conflict
                    else "fill_gaps_policy"
                )
            ...
            for description in response.descriptions:
                ...
                low = description.confidence < config.min_ai_confidence
                if (
                    (key not in description_targets and not newly_accepted)
                    or (column is None and not newly_accepted)
                    or (
                        column is not None
                        and not _needs_description(column, config.override_existing)
                    )
                    ...
                    observer.event(
                        ...
                        request_id=request_id,
                        target=key,
                        status="deferred" if low else "rejected",
                        reason="below_confidence_threshold"
                        if low
                        else "unrequested_existing_or_unsupported",
                    )
                    continue
                if column is None:
                    ...
                    new_descriptions[key] = (description.description, description)
                else:
                    description_before.setdefault(key, dict(column))
                    _write_description(column, description.description, description, config.model)
            ...
            for proposal in response.table_descriptions:
                table = catalog_tables.get(proposal.dataset_id)
                low = proposal.confidence < config.min_ai_confidence
                if (
                    proposal.dataset_id not in table_targets
                    or not _needs_description(table, config.override_existing)
                    or not proposal.description.strip()
                    or low
                ):
                    observer.count("descriptions.tables.rejected")
                    observer.event(
                        "description.decided",
                        actor="description_policy",
                        request_id=request_id,
                        target=proposal.dataset_id,
                        kind="table",
                        status="deferred" if low else "rejected",
                        reason="below_confidence_threshold"
                        if low
                        else "unrequested_or_existing_description",
                    )
                    continue
                table_description_before.setdefault(proposal.dataset_id, dict(table))
                _write_description(table, proposal.description, proposal, config.model)
            ...
            for dataset_id, previous in table_description_before.items():
                catalog_tables[dataset_id].clear()
                catalog_tables[dataset_id].update(previous)
    ...
    if applied_columns or applied_tables:
        ...
        for key, (text, proposal) in new_descriptions.items():
            ...
            if _needs_description(column, config.override_existing):
                _write_description(column, text, proposal, config.model)
